...
import { countries } from '#shared/mock/data';
...
export const _tours = [...Array(12)].map((_, index) => {
    ...
    const gallery = [...Array(6)].map((__, itemIndex) => mock.image.travel(itemIndex + 2));

    const highlights = [...Array(6)].map((__, itemIndex) => mock.sentence(itemIndex));
    ...
    const program = [...Array(3)].map((__, itemIndex) => ({
        ...
        text: mock.description(itemIndex),
    }));
